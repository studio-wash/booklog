...
|---------|-------------|------|----------|--------|-----------|
| `db-latest-reading-entry` | `AppDatabase.latestReadingEntry()` — `created_at` 최신 1건 | ✅ 완료 | High | - | 20분 |
| `provider-current-reading` | 마지막 기록 책·누적 페이지 스냅샷 `FutureProvider` | ✅ 완료 | High | `db-latest-reading-entry` | 25분 |
| `ui-grass-current-reading-card` | `GrassScreen` 잔디 아래 카드·탭 → `/log?bookId=` | ✅ 완료 | High | `provider-current-reading` | 45분 |
| `test-current-reading-latest-book` | 최신 `created_at` 기준 책 선택 DB/통합 테스트 | ✅ 완료 | Medium | `db-latest-reading-entry` | 30분 |
| `spec-fr10-current-reading` | `booklog-mvp.md` FR-10 및 코드 매핑 행 추가 | ✅ 완료 | Medium | - | 15분 |
...
- 2026-05-12: `/code PLAN-000002 *` 완료 — DB·프로바이더·카드·테스트·spec/README 반영, `flutter analyze` / `flutter test` 통과.
