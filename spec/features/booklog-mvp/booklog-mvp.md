...
| ID | 요구사항 |
|----|----------|
| FR-1 | 사용자는 **책**을 등록·수정·삭제할 수 있다. 필드: 제목·**ISBN**(필수, DB에서 유일)·표지 URL(문자열, 빈 값 허용·UI는 플레이스홀더), 총 페이지(선택), 완독 한줄 평(선택). 네이버 검색으로 추가할 때 응답에 있는 **저자·출판사·설명·출간일 문자열·도서 링크** 등 가격·할인을 제외한 메타를 함께 저장한다. |
